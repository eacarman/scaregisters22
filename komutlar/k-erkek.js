...
let klanuyerol = db.fetch(`klanuyer_${message.guild.id}`)
...
member.roles.add(klanuyerol)
const kayıtolan = message.mentions.members.first() || message.guild.members.cache.get(args[0]) 

db.add(`klanuye+_${message.author.id}`, 1)

const embed = new discord.MessageEmbed()
.setThumbnail(message.author.displayAvatarURL({dynamic : true}))
.setAuthor(client.user.username, client.user.avatarURL())  
.setColor("#f1ebeb")
.setDescription(`${member} Kullanıcının ismi \`${tag} ${isim}\` olarak değiştirildi ve <@&${klanuyerol}> rolü verildi!`)
